...
class SettingsCubit extends Cubit<SettingsState> {
  ...
  SettingsCubit(
      ...
      : super(const SettingsState(settings: defaultSettings));
  ...
  Future<void> updateOneSettingWithKey({
    ...
    print(state);
    emit(LoadingSettingsState(settings: state.settings));
    final isUpdated =
  ...
  SettingsState _mapSettingsOrFailureToSettingsState(
      Either<Failure, UserSettings> settingsOrFailure) {
    return settingsOrFailure.fold(
      (failure) => FailureSettingsState(
          message: mapFailureToMessage(failure), settings: state.settings),
      (settings) {
        print(settings);
        print("s");
